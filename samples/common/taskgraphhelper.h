...
namespace task_scheduler {
    ...
    template <class TMemInterface>
    class base_task_graph_helper : public TMemInterface {
    public:
        ...
        : task_graph(_task_graph)
    {
     ...
     template <class TMemInterface>
    void base_task_graph_helper<TMemInterface>::load(string_type _file_name)
    {
        ...
        string_type _str)
    {
        ...
        case TaskPriority: {
            ...
            for (auto i = 0; i < task_type::num_priority; i++) {
                if (_str.compare(_task->debug.priority_to_string(static_cast<typename task_type::priority_selector>(i))) == 0) {
